...
export async function GET(request: Request) {
  try {
    ...
    const wordSearches = activities.filter(a => a.activity_type === "word_search");
    const audioGenerations = activities.filter(a => a.activity_type === "audio_generation");
    ...
    const averageResponseTime = activities.length > 0 
      ? Math.round(activities.reduce((sum, a) => sum + a.response_time_ms, 0) / activities.length)
      : 0;
